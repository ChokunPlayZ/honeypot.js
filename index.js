    ...
    (err, out) => {
      if (err)
        return console.error(
          `Something went wrong while generating RSA keys: ${err}`
        );
...
function main() {
  new ssh2.Server(
      ...
      client
        .on("authentication", function (ctx) {
          if (ctx.method === "password") {
            fs.appendFile(
              "log.txt",
              `${info.ip}|${ctx.username}:${ctx.password}\n`,
              function (err) {
            ...
            if (falsePositive) {
              console.log(
                `${client._sock.remoteAddress} >> '${ctx.username}' | '${ctx.password}' - ACCEPTED`
              );
              logToDiscord([
                {
                  name: "Type",
                  value: `Authentication Attempt (ACCEPTED)`,
                  inline: false,
                },
                { name: "IP", value: `${info.ip}` },
                { name: "Username", value: `${ctx.username}` },
                { name: "Password", value: `${ctx.password}` },
              ]);
              ctx.accept();
            } else {
              console.log(
                `${client._sock.remoteAddress} >> '${ctx.username}' | '${ctx.password}' - REJECTED`
              );
              logToDiscord([
                {
                  name: "Type",
                  value: `Authentication Attempt (REJECTED)`,
                  inline: false,
                },
                { name: "IP", value: `${info.ip}` },
                { name: "Username", value: `${ctx.username}` },
        ...
        .on("ready", function () {
          console.log(
            `${client._sock.remoteAddress} >>> Authenticated (Not really of course :p)`
          );
          logToDiscord(`[${info.ip}] Authenticated (Not really of course :p)`);
          logToDiscord([
            {
              name: "Type",
              value: `Authenticated (Not Really Suckers)`,
              inline: false,
            },
            { name: "IP", value: `${info.ip}` },
          ]);
    ...
    logToDiscord([{ name: "Type", value: `Status: Listening`, inline: false }]);
  });
}
